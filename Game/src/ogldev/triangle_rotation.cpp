...
private:
	...
	int Init(ESContext *esContext)
	{
		...
		char *vertexShader = getTriangleRotationVertexShader();
		char *fragmentShader = getTriangleRotationFragmentShader();
		...
		userData->Demo->uRotation = glGetUniformLocation( userData->Demo->programObject, "u_rotation" );

		if (userData->Demo->uRotation <= -1)
		{
			esLogMessage("Error getting userData->u_rotation:\n%d\n", userData->Demo->uRotation);
			return FALSE;
		}
	...
	static void Draw(ESContext *esContext)
	{
		...
		TransposeArray(RotationMatrix, &rows, &columns);
		 ...
		 */
		// glUniformMatrix4fv(userData->Demo->uRotation, 1, GL_TRUE, RotationMatrix);
		glUniformMatrix4fv(userData->Demo->uRotation, 1, GL_FALSE, RotationMatrix);
...
public:
	int Start(int argc, char *argv[])
	{
		...
		esCreateWindow2(&esContext, "OGLDEV Triangle Rotation", WINDOW_ICON, 720, 720, ES_WINDOW_RGB | ES_WINDOW_DEPTH);

		if(!this->Init(&esContext))
			return GAME_ERROR_INIT_FAILED;

		esRegisterDrawFunc(&esContext, TriangleRotation::Draw);
...
GLuint TriangleRotation::VBO = 0;

int RunOglDevGameTriangleRotation(int argc, char *argv[])
{
	TriangleRotation triangleRotation;
	return triangleRotation.Start(argc, argv);
}